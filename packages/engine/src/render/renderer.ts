 ...
 * the dynamic entities — fast enough for mid-range phones.
 */
export class IsoRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, sim: Simulation) {
    ...
    this.initChunks();
    this.initStars();
    sim.world.onColumnChange = (x, y) => this.invalidateColumn(x, y);
    const r = sim.rover;
    this.camera.centerOnTile(r.pos.x, r.pos.y, sim.world.height(r.pos.x, r.pos.y));
  }
  ...
  private invalidateColumn(x: number, y: number): void {
    const cx = Math.floor(x / CHUNK);
    const cy = Math.floor(y / CHUNK);
    for (let oy = -1; oy <= 0; oy++) {
      for (let ox = -1; ox <= 0; ox++) {
        // Exposure of neighbours can change; be safe near chunk borders.
        const nx = cx + ox;
        const ny = cy + oy;
        if (nx < 0 || ny < 0) continue;
        const onBorderX = ox === -1 && x % CHUNK !== 0;
        const onBorderY = oy === -1 && y % CHUNK !== 0;
        if (ox === -1 && onBorderX) continue;
        if (oy === -1 && onBorderY) continue;
        const chunk = this.chunks[ny * this.chunksPerSide + nx];
  ...
  private rebuildChunk(chunk: Chunk): void {
    ...
    const xN = Math.min(CHUNK, world.size - x0);
    const yN = Math.min(CHUNK, world.size - y0);

    // Painter order: diagonal rows (x+y ascending), columns bottom-to-top.
    for (let s = 0; s <= xN + yN - 2; s++) {
      for (let lx = Math.max(0, s - yN + 1); lx <= Math.min(s, xN - 1); lx++) {
        const ly = s - lx;
        const x = x0 + lx;
        const y = y0 + ly;
        const h = world.height(x, y);
        if (h < 0) continue;
        const hRight = world.height(x + 1, y); // -1 outside => exposed
        const hLeft = world.height(x, y + 1);
        for (let z = 0; z <= h; z++) {
          ...
          ctx.drawImage(
            this.sprites.tile(m, x, y, z) as CanvasImageSource,
  ...
  draw(): void {
    ...
    // Sky.
    ctx.fillStyle = mixColor(sim.body.palette.skyNight, sim.body.palette.sky, daylight);
    ...
    if (camera.follow) {
      const p = project(rover.renderPos.x, rover.renderPos.y, rz);
    ...
    if (world.isSolid(fx, fy)) {
      this.strokeTileDiamond(fx, fy, world.height(fx, fy), 'rgba(255,255,255,0.35)');
    }
    if (rover.mining) {
      const m = rover.mining;
      const pulse = 0.35 + (Math.sin(sim.time * 10) + 1) * 0.2;
      this.strokeTileDiamond(m.pos.x, m.pos.y, world.height(m.pos.x, m.pos.y), `rgba(255,179,71,${pulse})`);
    }

    // Entities in diagonal order.
    type Ent = { s: number; draw: () => void };
    const ents: Ent[] = [];
    for (const a of sim.anomalies) {
      const az = world.height(a.pos.x, a.pos.y);
      if (az < 0) continue;
      const p = camera.toScreen(...projXY(a.pos.x, a.pos.y, az + 1));
      ents.push({
        s: a.pos.x + a.pos.y,
    ...
    for (const st of sim.structures) {
      ...
      const p = camera.toScreen(...projXY(st.pos.x, st.pos.y, sz + 1));
      ents.push({
        s: st.pos.x + st.pos.y,
    ...
    {
      const p = camera.toScreen(...projXY(rover.renderPos.x, rover.renderPos.y, rz + 1));
      ents.push({
        s: rover.renderPos.x + rover.renderPos.y,
        draw: () => drawRover(ctx, p.x, p.y, camera.zoom, rover, daylight),
      });
    }
    ...
    this.redrawOccluders(rover.renderPos, rz);

    // Night tint.
    if (daylight < 1) {
      ctx.fillStyle = `rgba(8,10,26,${(1 - daylight) * 0.45})`;
      ctx.fillRect(0, 0, camera.viewW, camera.viewH);
    }
  }
  ...
  /**
   * Columns "in front of" (greater diagonal than) an entity can occlude it.
   * Chunk canvases were already stamped below entities, so re-stamp just
   * those columns above everything. Cheap: a few dozen sprite blits.
   */
  private redrawOccluders(pos: Vec2, z: number): void {
    const world = this.sim.world;
    const { ctx, camera } = this;
    const ex = Math.round(pos.x);
    const ey = Math.round(pos.y);
    const s0 = ex + ey;
    const R = 7;
    for (let s = s0 + 1; s <= s0 + R; s++) {
      for (let x = ex - R; x <= ex + R; x++) {
        const y = s - x;
        if (Math.abs(y - ey) > R) continue;
        const h = world.height(x, y);
        if (h <= z) continue; // cannot occlude
        const hRight = world.height(x + 1, y);
        const hLeft = world.height(x, y + 1);
        for (let zz = Math.max(0, Math.floor(z)); zz <= h; zz++) {
          const visible = zz === h || zz > hRight || zz > hLeft;
          if (!visible) continue;
          const m = world.get(x, y, zz);
          if (m === Material.Air) continue;
          const p = project(x, y, zz);
          const sc = camera.toScreen(p.sx - TILE_W / 2, p.sy - TILE_H / 2);
          ctx.drawImage(
            this.sprites.tile(m, x, y, zz) as CanvasImageSource,
            sc.x,
            sc.y,
            TILE_W * camera.zoom,
  ...
  private strokeTileDiamond(x: number, y: number, z: number, style: string): void {
    const { ctx, camera } = this;
    const p = project(x, y, z);
  ...
  /** Tile under a canvas-space point (top surfaces considered, front-most wins). */
  pickTile(canvasX: number, canvasY: number): Vec2 | null {
    const { camera } = this;
    const world = this.sim.world;
    const pr = camera.toProjected(canvasX, canvasY);
    // Invert at z=0 then walk down diagonals checking actual surface height.
    const bx = pr.px / (TILE_W / 2);
    const by = pr.py / (TILE_H / 2);
    const x0 = Math.round((by + bx) / 2);
    const y0 = Math.round((by - bx) / 2);
    let best: Vec2 | null = null;
    for (let k = world.maxHeight; k >= 0; k--) {
      // A column whose top is at z=k projects to this screen point if it
      // sits k * TILE_Z / (TILE_H/2) diagonal steps further away... walk it.
      const shift = (k * TILE_Z) / TILE_H;
      const cand = [
        { x: Math.round(x0 + shift), y: Math.round(y0 + shift) },
        { x: Math.floor(x0 + shift), y: Math.ceil(y0 + shift) },
        { x: Math.ceil(x0 + shift), y: Math.floor(y0 + shift) },
      ];
      for (const c of cand) {
        if (!world.inBounds(c.x, c.y)) continue;
        const h = world.height(c.x, c.y);
        if (h < 0) continue;
        const p = project(c.x, c.y, h);
  ...
  capturePhoto(): string | null {
    ...
    tmp.width = w;
    tmp.height = h;
    const saved = { cx: this.camera.cx, cy: this.camera.cy, zoom: this.camera.zoom, vw: this.camera.viewW, vh: this.camera.viewH, follow: this.camera.follow };
    ...
    try {
      ...
      const r = this.sim.rover;
      this.camera.centerOnTile(r.pos.x, r.pos.y, this.sim.world.height(r.pos.x, r.pos.y));
      this.draw();
      // Vignette + timestamp strip for that mission-photo feel.
      const c = this.ctx;
      ...
      c.fillText(`${this.sim.body.name.toUpperCase()}  T+${this.sim.time.toFixed(0)}s  [${r.pos.x},${r.pos.y}]`, 8, h - 8);
  ...
  /** Paint a minimap of the whole body onto the given canvas. */
